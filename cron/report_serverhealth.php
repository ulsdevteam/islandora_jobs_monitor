...
if ($server_id > 0) {
  $command = 'ps -ef | grep "/usr/bin/gearman"';
  $output = array();
  ...
  $command = "wget '" . $server_monitor . "islandora/islandora_job_monitor/api/serverhealth/?cpu=" . $cpu . "&memory=" . $mem . "&errors=" . $host_error_log_errors_count . "&disk_used_pct=" . $disk_used_pct . "&disk_bytes_avail=" . $bytes_avail . "&gearman_running=" . $gearman_running . "' >/dev/null 2>&1";
  $output = array();
  exec($command, $output, $return);
}
else {
  $command = 'ps -C "gearmand"';
  $output = array();
  exec($command, $output, $return);
  $gearman_running = (count($output) > 1) ? 1 : 0;
  ...
  $sql = "INSERT INTO `host_server_health` (`server_id`, `timestamp`, " .
         ...
         "`disk_used_pct`, `disk_bytes_avail`, `gearman_running`) VALUES (" .
        $server_id . ", now(), " . $cpu . ", " . $mem . ", " .
        $host_error_log_errors_count . ", '" . $ingest_tmp_disk_used_pct . "'," .
        $bytes_avail . ", " . $gearman_running . ")";
  $result = mysqli_query($link, $sql);
  if (!$result) {
